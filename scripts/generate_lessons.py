...
# Notebook ID from the workspace
NOTEBOOK_ID = "5a520c9c-d47f-4354-95b1-8d208079f55d"

# Force regeneration to bring all lessons in order
FORCE_OVERWRITE = True

# Full 26-week curriculum definition with their exact 10 blocks
WEEKLY_BLOCKS = {
    1: {
    ...
    4: {
        "title_ru": "Кейс-проект: Умный Telegram-фильтр и лид-парсер",
        "title_en": "Case Study: Telegram Lead Filter and Parser",
        "ru": [
    ...
    5: {
        "title_ru": "Продвинутый n8n: Массивы данных и подпроцессы",
    ...
    11: {
        "title_ru": "Собственный ReAct-агент с нуля на Python",
        "title_en": "Custom ReAct Agent from Scratch in Python",
        "ru": [
            ...
            "Блок 5 (ИИ-Инженер / Автоматизация): Динамические схемы — автоматическое построение JSON-схем инструментов из docstring.",
    ...
    12: {
        ...
        "title_en": "Case Study: Local CLI Agent with Tools",
        "ru": [
    ...
    14: {
        ...
        "title_en": "Advanced Crews and Custom Tools in CrewAI",
        "ru": [
    ...
    15: {
        "title_ru": "Граф-ориентированное программирование: LangGraph Foundations",
        "title_en": "Graph-Oriented Programming: LangGraph Foundations",
        "ru": [
    ...
    17: {
        ...
        "title_en": "Advanced Enterprise RAG for Production",
        "ru": [
    ...
    18: {
        "title_ru": "Наблюдаемость и Трейсинг в LangSmith",
        "title_en": "Observability and Tracing in LangSmith",
        "ru": [
    ...
    19: {
        ...
        "title_en": "Quality Evaluations and Regression Harness",
        "ru": [
    ...
    20: {
        "title_ru": "Дисциплина состояния и Окончание сессии",
        "title_en": "State Discipline and Session Handover",
        "ru": [
            ...
            "Блок 5 (ИИ-Инженер / Автоматизация): Бэкап состояний — architecture резервного копирования и быстрого восстановления состояний.",
    ...
    22: {
        ...
        "title_en": "Deployment and Durable Execution",
        "ru": [
    ...
    24: {
        ...
        "title_en": "Capstone Project and Career Launch",
        "ru": [
...
def clean_lesson_content(text: str, lang: str) -> str:
    if not text:
        return text
        
    # Unpack JSON if returned inside text
    content = text.strip()
    ...
    if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
        ...
        try:
            ...
            answer = parsed.get("value", {}).get("answer") or parsed.get("answer")
            if answer:
                content = answer.strip()
        except Exception:
            pass
            
    # Replace raw filenames (PDF, .md, backtick-wrapped) with human-readable localized phrases
    if lang == "ru":
        named_replacements = [
            ...
            (r"Learn Harness Engineering", "курс по Harness-инженерии"),
        ...
        named_replacements = [
            ...
            (r"Learn Harness Engineering", "Harness Engineering course"),
            # Specific known filenames
    ...
    # Replace any remaining generic .md filenames
    content = re.sub(r"\b[A-Za-z0-9_\-]+\.md\b", "", content)
    # Replace any other generic `.pdf` filename patterns with a clean version without extension
    content = re.sub(r"\b([a-zA-Z0-9_-]+)\.pdf\b", r"\1", content)
    
    # Strip bracketed footnotes like [1], [2], [1, 2]
    content = re.sub(r"\[\d+(?:\s*,\s*\d+)*\]", "", content)
    # Strip separated footnotes like [1] , [2]
    content = re.sub(r"\[\d+\]\s*,\s*\[\d+\]", "", content)
    
    # Format plain website links to be clickable Markdown links if they are not already
    ...
    def link_replacer(match):
        ...
        if "anthropic.com" in clean_url:
            domain = "Anthropic Research" if lang == "en" else "Исследования Anthropic"
        elif "langchain.com" in clean_url or "langchain" in clean_url:
            domain = "LangChain Docs" if lang == "en" else "Документация LangChain"
        elif "n8n.io" in clean_url:
            domain = "n8n Docs" if lang == "en" else "Документация n8n"
        elif "github.com" in clean_url:
            domain = "GitHub Repository" if lang == "en" else "Репозиторий GitHub"
        elif "postman.com" in clean_url:
            domain = "Postman Learning" if lang == "en" else "Обучение Postman"
        elif "zapier.com" in clean_url:
            domain = "Zapier Guide" if lang == "en" else "Руководство Zapier"
            
        return f"[{domain}]({clean_url}){punctuation}"
        
    content = re.sub(url_pattern, link_replacer, content)
...
def run_nlm_query(prompt: str) -> str:
    """Helper to run nlm CLI query command with safety sleep."""
    time.sleep(12) # Cooldown to respect Google Rate Limits
    ...
    try:
        ...
        print(f"Error executing nlm query: {e.stderr}", file=sys.stderr)
        return ""
    except Exception as e:
        print(f"Unexpected error: {e}", file=sys.stderr)
        return ""

def generate_weekly_content(week_num: int, lang: str):
    """Queries NotebookLM and generates detailed markdown lesson materials for a specific week."""
    week_data = WEEKLY_BLOCKS.get(week_num)
    if not week_data:
        print(f"Week {week_num} not found in configuration!")
        return

    week_id = f"week-{week_num}"
    title = week_data["title_ru"] if lang == "ru" else week_data["title_en"]
    blocks = week_data["ru"] if lang == "ru" else week_data["en"]
    
    # Format the 10 blocks as a numbered list
    blocks_formatted = "\n".join(f"{i+1}. {block}" for i, block in enumerate(blocks))
    
    print(f"\n--- Starting 10-Block Generation for {week_id} in {lang.upper()} ---")

    # Target directory
    base_dir = os.path.abspath(os.path.join(
        os.path.dirname(__file__), "..", "src", "data", "lessons", lang, week_id
    ))
    os.makedirs(base_dir, exist_ok=True)

    # 1. GENERATE HIGH-DENSITY LECTURE (10-BLOCK STRUCTURE - BLOCK-BY-BLOCK GENERATION)
    lecture_file = os.path.join(base_dir, "lecture.md")
    if FORCE_OVERWRITE or not os.path.exists(lecture_file) or os.path.getsize(lecture_file) < 100000:
        lecture_blocks = []
        for i, block in enumerate(blocks):
            block_num = i + 1
            print(f"-> Querying 10-Block Lecture Block {block_num}/10 for {week_id} ({lang.upper()})...")
            block_prompt = (
                f"You are a leading AI Automation Architect and Educator. Grounded directly and exhaustively in the notebook sources, "
                f"write an EXTREMELY DETAILED, EXPANSIVE, AND VOLUMINOUS educational chapter for our professional course on AI Automation & Agent Builders.\n\n"
                f"This is Chapter {block_num} of Week {week_num} ('{title}').\n"
                f"Topic: {block}\n\n"
                f"Write an exhaustive, comprehensive, production-grade deep-dive of at least 1500 words for this specific block containing:\n"
                f"- Deep theoretical analysis and core concepts.\n"
                f"- Detailed step-by-step practical guides, schemas/ASCII architecture diagrams, and complete working code blocks.\n"
                f"- Realistic business applications (how this block is implemented in companies).\n"
                f"- Edge-cases, common errors, rate limits, and debugging loops.\n\n"
                f"Make the writing style highly engaging, pedagogical, and clear. Ensure every workspace source relevant to this block "
                f"is fully cited and referenced. Quote materials, authors, and files from: 'agent_roadmap_ru.pdf', 'ai_builder_ru.pdf', "
                f"vc.ru/Habr articles, and the 12 Harness Engineering Lectures to prove depth. DO NOT summarize or omit details — write out everything fully.\n\n"
                f"Write this entire chapter in " + ("Russian" if lang == "ru" else "English") + ". "
                f"Start directly with the content. Use clean GitHub markdown with GFM tables and alerts."
            )
            block_content = run_nlm_query(block_prompt)
            if block_content:
                block_content = clean_lesson_content(block_content, lang)
                if not block_content.startswith("#") and not block_content.startswith("##"):
                    block_content = f"## {block}\n\n" + block_content
                lecture_blocks.append(block_content)
                
                # Save incrementally so the user/system can see progress in real-time!
                full_lecture = f"# Лекция {week_num}. {title}\n\n" + "\n\n---\n\n".join(lecture_blocks)
                with open(lecture_file, "w", encoding="utf-8") as f:
                    f.write(full_lecture)
                print(f"✓ Saved Lecture block {block_num}/10 for {week_id} ({lang.upper()}) (Incremental)")
        
        # Normalize headings after all blocks are saved
        try:
            normalizer_path = os.path.join(os.path.dirname(__file__), "normalize_lectures.py")
            result = subprocess.run(
                ["python3", normalizer_path],
                capture_output=True, text=True, cwd=os.path.dirname(os.path.dirname(__file__))
            )
            if result.returncode == 0:
                print(f"✓ Headings normalized for {week_id} ({lang.upper()})")
            else:
                print(f"  Warning: normalizer stderr: {result.stderr[:200]}", file=sys.stderr)
        except Exception as e:
            print(f"  Warning: could not run normalizer: {e}", file=sys.stderr)
    else:
        print(f"  Lecture for {week_id} ({lang.upper()}) already exists, skipping.")


    # 2. GENERATE SLIDES (10-BLOCK ALIGNMENT)
    slides_file = os.path.join(base_dir, "slides.md")
    if FORCE_OVERWRITE or not os.path.exists(slides_file) or os.path.getsize(slides_file) < 10000:
        slides_prompt = (
            f"Based on the notebook resources, create a structured slide-by-slide presentation outline "
            f"for Week {week_num}: '{title}'. "
            f"You must create exactly 10 slides, with EACH slide mapping precisely to one of the following 10 blocks:\n"
            f"{blocks_formatted}\n\n"
            f"For each slide, write:\n"
            f"- Slide Title (e.g. '📊 Слайд X. [Название блока]' or '📊 Slide X. [Block Name]')\n"
            f"- Visual Layout Concept (e.g., specific HSL dark/light modes, schemas, Lucide icons to use)\n"
            f"- Key bullet points outlining the core technical facts.\n"
            f"Delineate slides using '---' horizontal lines.\n\n"
            f"Write this entire slide presentation in " + ("Russian" if lang == "ru" else "English") + ". "
            f"Start directly with the first slide."
        )
        
        print(f"-> Querying 10-Block Slides for {week_id} ({lang.upper()})...")
        slides_content = run_nlm_query(slides_prompt)
        if slides_content:
            slides_content = clean_lesson_content(slides_content, lang)
            if not slides_content.startswith("#"):
                slides_content = f"# Презентация: {title}\n\n" + slides_content
                
            with open(slides_file, "w", encoding="utf-8") as f:
                f.write(slides_content)
            print(f"✓ Saved Slides for {week_id} ({lang.upper()})")
    else:
        print(f"  Slides for {week_id} ({lang.upper()}) already exists, skipping.")

    # 3. GENERATE VIDEO SCRIPT (10-BLOCK ALIGNMENT)
    video_file = os.path.join(base_dir, "video.md")
    if FORCE_OVERWRITE or not os.path.exists(video_file) or os.path.getsize(video_file) < 10000:
        video_prompt = (
            f"Based on the notebook sources, write a professional spoken video script/teleprompter script "
            f"for Week {week_num}: '{title}'. "
            f"Structure the script into 10 scenes, with EACH scene mapping precisely to one of the following 10 blocks:\n"
            f"{blocks_formatted}\n\n"
            f"Format it beautifully as a markdown script using GFM tables with columns:\n"
            f"- 'Кадр / Scene' (describing what is shown on screen, e.g., speaker close-up, n8n visual flow, Python code block)\n"
            f"- 'Звуковое сопровождение / Voiceover' (the exact word-for-word spoken text, explaining the block clearly to tech-literate students).\n\n"
            f"Write this entire script in " + ("Russian" if lang == "ru" else "English") + ". "
            f"Start directly with the script."
        )
        
        print(f"-> Querying 10-Block Video Script for {week_id} ({lang.upper()})...")
        video_content = run_nlm_query(video_prompt)
        if video_content:
            video_content = clean_lesson_content(video_content, lang)
            if not video_content.startswith("#"):
                video_content = f"# Сценарий видео: {title}\n\n" + video_content
                
            with open(video_file, "w", encoding="utf-8") as f:
                f.write(video_content)
            print(f"✓ Saved Video Script for {week_id} ({lang.upper()})")
    else:
        print(f"  Video Script for {week_id} ({lang.upper()}) already exists, skipping.")

def main():
    print("Initializing NotebookLM High-Density 10-Block Lesson Generation (Safe Parallel Mode)...")
    
    # We support targeting specific weeks or running sequentially
    target_weeks = [1, 2, 3, 4, 5, 6, 7, 8, 11, 15, 18, 20] # Target weeks currently defined for production lessons
    
    # Optionally, read command line argument if targeting single week
    if len(sys.argv) > 1:
        try:
            target_weeks = [int(x) for x in sys.argv[1].split(",") if x.strip().isdigit()]
        except ValueError:
            print("Invalid week argument. Use comma separated numbers like: 1,2,3")
            sys.exit(1)

    print(f"Generating lessons for weeks: {target_weeks}")
    
    tasks = []
    for week_num in target_weeks:
        for lang in ["ru", "en"]:
            tasks.append((week_num, lang))
            
    # Run tasks in parallel using ThreadPoolExecutor
    # 3 concurrent workers provides optimal balance of speed and API stability
    max_workers = 3
    print(f"Spawning concurrent workers: {max_workers} (Safe Parallel Mode)")
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(generate_weekly_content, week_num, lang): (week_num, lang) for week_num, lang in tasks}
        
        for future in concurrent.futures.as_completed(futures):
            week_num, lang = futures[future]
            try:
                future.result()
                print(f"✓ Thread completed generation for week-{week_num} ({lang.upper()})")
            except Exception as e:
                print(f"✗ Thread error generating week-{week_num} ({lang.upper()}): {e}", file=sys.stderr)
                
    print("\n🎉 Batch Lesson Generation (Safe Parallel Mode) Completed successfully!")
